...
class myBitSet{
    ...
    static const size_t _M_offset = _Nw % _BITS_PER_WORD;
    static const size_t _M_sub_offset = _BITS_PER_WORD - _M_offset;

    _WordT _M_w[_M_PhysicLength];
    mutable std::vector<int> _M_Indices;
    mutable int _M_BitCount = -1;

    //Override ostream & operator <<
    template <size_t st>
    friend std::ostream & operator <<(std::ostream & os, const myBitSet<st> &rhs);
    ...
    size_t _M_do_word_index(size_t bitIndex)
    { return bitIndex >> _ADDRESS_BITS_PER_WORD; }

    size_t _M_do_word_index(size_t bitIndex) const
    { return bitIndex >> _ADDRESS_BITS_PER_WORD; }

    void _M_do_sanitize(){
        if(_M_offset != 0){
            (_M_w[_M_PhysicLength - 1] <<= _M_sub_offset) >>= _M_sub_offset;
        }
    }
    ...
    void _M_do_right_shift(size_t __shift){
        if( __shift !=0 ){
            //__wshift 是数组元素移动的个数
            //__offset 是对于不够一个Word的需要对其移位
            const size_t __wshift = __shift >> _ADDRESS_BITS_PER_WORD;
            const size_t __offset = __shift % _BITS_PER_WORD;
            const size_t __limit  = _M_PhysicLength - __wshift -1;
            ...
            if(__offset == 0)
                ...
                const size_t __sub_offset = _BITS_PER_WORD - __offset;
                for(size_t __n = 0; __n < __limit; ++__n)
                    _M_w[__n] = (_M_w[__n + __wshift] >> __offset) |
                        (_M_w[__n + __wshift + 1] << __sub_offset);
                _M_w[__limit] = _M_w[_M_PhysicLength - 1] >> __offset;
            }

            //补足最右边位
            memset(_M_w + __limit + 1, 0, sizeof(_WordT) * ((_M_w + _M_PhysicLength) - (_M_w + __limit + 1)));
        }
    }
    void _M_do_left_shift(size_t __shift){
        if( __shift !=0 ){
            ...
            if(__offset == 0)
                for(size_t __n = _M_PhysicLength - 1; __n >= __wshift; --__n)
                    _M_w[__n] = _M_w[__n - __wshift];
            ...
            //补足最左边位
            memset(_M_w + 0, 0, sizeof(_WordT) * ((_M_w + __wshift) - (_M_w + 0)));
        }
    }
    ...
    myBitSet(_WordT __initW){
        _M_w[0] = __initW;
        for(size_t i=1; i<_M_PhysicLength; i++){
            _M_w[i] = 0;
        }
    }
    ...
    {
        size_t _w_Index = _M_do_word_index(_pos);
        _pos %= _BITS_PER_WORD;
    ...
    myBitSet operator<<(size_t _pos){
        return myBitSet(*this) <<= _pos;
    }
    myBitSet operator<<(size_t _pos)const{
        return myBitSet(*this) <<= _pos;
    }
    myBitSet & operator<<=(size_t _pos){
        _pos %= _Nw;
        _M_do_left_shift(_pos);
        _M_do_sanitize();
        return *this;
    }
    ...
    _WordT operator[](size_t _pos) const
    {
        size_t _w_Index = _M_do_word_index(_pos);
        _pos %= _BITS_PER_WORD;
        return _M_w[_w_Index] & (1ul << _pos);
...
template <size_t st>
std::ostream & operator <<(std::ostream & os, const myBitSet<st> &rhs)
{
    for(int i=rhs._M_LogicLength-1; i>=0; --i){
        if(rhs.test(i))
            os<<"1 ";
        else
            os<<"0 ";
